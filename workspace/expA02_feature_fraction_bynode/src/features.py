...
expA01: expA00の22特徴量に加え、下半身6キーポイント(インデックス11-16)の
bbox正規化座標(x, y)と信頼度を直接追加（+18次元, 計40次元）。
信頼度 < 0.3 のキーポイントは座標をNaNとして欠損扱いにする。
"""
...
def extract_features(data: dict) -> dict:
    ...
    # === 追加18特徴量: 下半身6KPのbbox正規化生座標 + 信頼度 ===
    # 信頼度 < 0.3 の場合、座標は不信頼なので NaN に置換
    # LightGBM はNaNを欠損として自動処理（追加実装不要）
    lower_kp_names = [
        ("left_hip", LEFT_HIP),
        ("right_hip", RIGHT_HIP),
        ("left_knee", LEFT_KNEE),
    ...
    for name, idx in lower_kp_names:
        conf = float(scores[idx])
        x_norm = float(rel_kps[idx, 0])
        y_norm = float(rel_kps[idx, 1])
        features[f"raw_{name}_x"] = x_norm if conf >= 0.3 else float("nan")
        features[f"raw_{name}_y"] = y_norm if conf >= 0.3 else float("nan")
        features[f"raw_{name}_conf"] = conf
